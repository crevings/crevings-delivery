...
// Success draws its own SVG ring; idle has green focus ring; error has red ring + shake
const RING = {
  idle: "focus-visible:ring-2 focus-visible:ring-[#00bd6f]/50 focus-visible:border-[#00bd6f]",
  success: "border-[#00bd6f]",
  error: "ring-2 ring-rose-500/70 border-rose-500",
} as const;

const SUCCESS_COLOR = "#00bd6f";
...
const SLOT_CLASS =
  "bg-slate-50 border border-slate-200 text-center font-bold text-transparent caret-transparent outline-none transition-all duration-200 selection:bg-transparent disabled:cursor-not-allowed disabled:opacity-50 shadow-sm";
...
export function OtpInput({
  ...
  const commit = (next: string[]) => {
    ...
    onChange?.(code);
    if (next.every(Boolean) && next.length === length) onComplete?.(code);
  };
  ...
  const focusAt = (index: number) => {
    const targetIdx = Math.min(Math.max(index, 0), length - 1);
    const input = inputs.current[targetIdx];
    input?.focus();
    input?.select();
  };
  ...
  return (
    <div
      data-slot="otp-input"
      data-status={status}
      className={`relative inline-flex justify-center w-full ${className}`}
      {...props}
    >
      <motion.div
        onFocus={(event) => {
        ...
        className={`relative flex items-center justify-center ${scale.gap}`}
      >
        {slots.map((slot, index) => (
          ...
          >
            <input
              ...
              className={`${SLOT_CLASS} ${scale.box} ${scale.text} ${RING[status]} ${slotClassName}`}
            />
              ...
              <AnimatePresence initial={false} custom={cleared}>
                {slot && (
                  <motion.span
                    ...
                    className={`font-bold text-slate-900 ${scale.text}`}
                  >
                    {mask ? "•" : slot}
                  </motion.span>
                )}
        ...
        {caretVisible && (
          <motion.span
            ...
            className={`pointer-events-none absolute left-0 top-1/2 w-0.5 rounded-full bg-[#00bd6f] ${scale.caret}`}
          />
        )}
      </motion.div>
    </div>
